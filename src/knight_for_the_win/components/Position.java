...
public class Position {
	 ...
	 *             - occurs when the params x and y are not legal
	 */
	public Position(int x, int y) throws PositionException {
		...
		if (!isPositionOnBoard(x, y) || !isLegalStartPosition(x, y)) {
			throw new PositionException("You can`t start on the final " + "or stuck position: " + x + " " + y);
		}
	...
	/**
	 * Checks if the starting position is not already the winning position or if
	 * it`s stuck position.
	 */
	private boolean isLegalStartPosition(int x, int y) {
		if ((x == STUCK_POSITION && y == STUCK_POSITION) || (x == WIN_POSITION && y == WIN_POSITION)) {
			return false;
		}
		return true;
	}
	...
	/**
	 * Checks if the user has made a position between the game rules
	 * 
	 * @param x
	 *            - row
	 * @param y
	...
	public boolean isLegalPosition(int x, int y) {
		if (isPositionOnBoard(x, y)) {
			// deltaX and deltaY are representation of the absolute value of the
	...
	/**
	 * Checks if the coordinates of x and y are equal with the winning
