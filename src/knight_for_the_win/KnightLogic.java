 ...
 *
 */
public class KnightLogic extends ChessLogicBase {
	...
	/**
	 * Creating a starting position of the player and draws board with the
	 * position in the console. this function can be used also for reseting the
	 * game.
	 * 
	...
	/**
	 * Converts the user String with position to Integers x and y. Checks if the
	 * new position is legal or is it a winning one. Contains {@link #position}.
	 * 
	 * @param userCoord
	 *            a String containing the int representation to be parsed
	 * @param WIN_MESSAGE
	 *            a String used only if user have reached the final point.
	 * @param ENTER_POSITION_MESSAGE
	 *            a String used as a hint for the player to enter new position.
	...
	public void moveKnight(String[] userCoord) {
		try {
			...
			System.out.println(
					"Only Integers accepted, number of required int: 2, current symbols inserted: " + e.getMessage());
		} finally {
			Board.getInstance().drawBoard(position);
		}
	...
	/**
	 * 
	 * @return boolean - true if the game has finished, false if the game
	 *         continues.
	 */
	@Override
